...
leafcutter.table.counts <- read_tsv("../code/rna_seq/SplicingAnalysis/leafcutter/GRCh38_GencodeRelease44Comprehensive/juncTableBeds/JuncCounts.sorted.bed.gz") %>%
    ...
    dplyr::select(1:6, Strand, Gene_name, Intron_coord, Annot, Coding, UTR, GencodePC, everything())

leafcutter.table.counts %>%
    dplyr::select(-c(1:3), -c(5:13)) %>%
...
And also, I want to compare the Sara's monosome vs heavy polysome, both with the FalconTube high yield and the low yield samples separately.

```{r}
UnpublishedPolysomeProfiling.pca$P.dat %>%
    mutate(Group = recode(Description,
        "S3_MonosomeFalconTubeHighYield" = "NMD_enriched",
        "S3_HeavyPolysomeFalconTubeHighYield" = "Control",
    )) %>%
    filter(Group %in% c("Control", "NMD_enriched")) %>%
    dplyr::select(sample, Group) %>%
    arrange(Group) %>%
    write_tsv("../code/config/contrast_groupfiles/SaraUnpublishedPolysomeProfiling_Monosome_vs_HeavyPolysome_FalconTubeHighYield.txt", col_names = F)

UnpublishedPolysomeProfiling.pca$P.dat %>%
    mutate(Group = recode(Description,
        "S3_Monosome" = "NMD_enriched",
        "S3_HeavyPolysome" = "Control",
    )) %>%
    filter(Group %in% c("Control", "NMD_enriched")) %>%
    dplyr::select(sample, Group) %>%
    arrange(Group) %>%
    write_tsv("../code/config/contrast_groupfiles/SaraUnpublishedPolysomeProfiling_Monosome_vs_HeavyPolysome_LowYield.txt", col_names = F)

```

And also contrast just the S3 monosome

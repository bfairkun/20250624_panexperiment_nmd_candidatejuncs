...
Joined_metadata <- bind_rows(
    ...
    DylansDatasets %>%
        ...
        #Edit filepaths for Dylan's unpublished datasets
        mutate(R1 = str_replace(R1, "/project/yangili1/dylan_stermer/.+/(.+?.fastq.gz)$", "/project2/yangili1/bjf79/Fastq/20250605_nuclearRNA/\\1")) %>%
        mutate(R2 = str_replace(R2, "/project/yangili1/dylan_stermer/.+/(.+?.fastq.gz)$", "/project2/yangili1/bjf79/Fastq/20250605_nuclearRNA/\\1")) %>%
        mutate(Platform = "ILLUMINA"),
    PolysomeProfiling %>%
        mutate(cell_type = "LCL") %>%
        mutate(Approach = "TranslationProfiling") %>%
...
read_tsv("../code/config/samples.links_filled.tsv") %>%
    mutate(R1 = case_when(
        StudyFirstAuthor == "UnpublishedFractionation" ~ str_replace(R1, "/project/yangili1/dylan_stermer/.+/(.+?.fastq.gz)$", "/project2/yangili1/bjf79/Fastq/20250605_nuclearRNA/\\1"),
        TRUE ~ R1
    ),
    R2 = case_when(
        StudyFirstAuthor == "UnpublishedFractionation" ~ str_replace(R2, "/project/yangili1/dylan_stermer/.+/(.+?.fastq.gz)$", "/project2/yangili1/bjf79/Fastq/20250605_nuclearRNA/\\1"),
        TRUE ~ R2
    )) %>%
    write_tsv("../code/config/samples.links_filled.tsv")
